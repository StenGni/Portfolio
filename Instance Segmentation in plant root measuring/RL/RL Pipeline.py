...
def pixel_to_mm(coords_px, original_image_shape, plate_size_mm=150, offset_px_x=0):
    ...
    offset_mm_x = offset_px_x * factor - 5
...
# --- RL MAIN INTEGRATION LOOP ---

if __name__ == "__main__":
    model_cv = load_model("../datalab_tasks/task5/saved_modelr4.h5", custom_objects={"f1": f1})
    model_rl = PPO.load(r"C:\Users\mstan\OneDrive\Dokumenty\GitHub\2024-25b-fai2-adsai-MonikaStangenberg231648\RL\models\RL_local\run_9oxszdtm\step_3000000.zip")
    ...
    mask, (offset_x, offset_y), left_in_cut = advanced_image_processing(image_path, model_cv)
    ...
    for i, target in enumerate(targets):
        ...
        print(f"[✓] Inoculated root tip {i+1} in {step_count} steps.")
        env.inoculate()

    # --- Return to starting position ---
    print(f"[INFO] Returning to starting position: {start_position}")
    env.target_position = start_position
    ...
    while not done:
        ...
        obs, reward, done, truncated, _ = env.step(action)
        if step_count % 100 == 0:
            print(f"Return Step {step_count}, Reward: {reward:.4f}")
        step_count += 1
        time.sleep(0.05)
